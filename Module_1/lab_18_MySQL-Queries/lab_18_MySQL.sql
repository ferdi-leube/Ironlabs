...
# 2
select prime_genre, count(ï»¿id) as num_aps  from applestore
group by prime_genre
order by count(ï»¿id) desc
limit 1;

# if you want to use/ count all the rows, mark that with a star: count(*)
...
## Question 6: apps which have the highest amounts of ratings are most likely to have the highest amount of users, the more users the more people who are ready to comment on the app
## Question 7: # 5 gives us the top 10 best rated apps, just because they are best rated doesn't mean they are popular or that good, if only one person gives a 5 star rating, the app will appear as rated with 5 stars, or the app is so unpopular, unsatisfied users won't even care to comment

select track_name, rating_count_tot, user_rating from applestore
order by rating_count_tot desc
limit 10; 

## Question 8: shows you that the most rated/most popular apps are not necesarily the best rated and the other way around
...
## Question 10: do people care about the price of an app?
